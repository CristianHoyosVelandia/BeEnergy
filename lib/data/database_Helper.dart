...
class DatabaseHelper {
  ...
  void onCreate(Database db, int version) async {
    // se crean las tablas
    await db.execute(
        'CREATE TABLE IF NOT EXISTS $tbUsuarioLogIn(idUser INTEGER PRIMARY KEY, nombre TEXT, lastname TEXT, telefono TEXT, correo TEXT, clave TEXT, energia TEXT, dinero TEXT, idCiudad INTEGER, role INTEGER, roleName TEXT, primaryColor TEXT, secondColor TEXT, urlImg TEXT, communityId INTEGER, communityName TEXT);');
    await db.execute(
        'CREATE TABLE IF NOT EXISTS $tbUsuarios    (idUser INTEGER PRIMARY KEY, nombre TEXT, lastname TEXT, telefono TEXT, correo TEXT, clave TEXT, energia TEXT, dinero TEXT, idCiudad INTEGER, role INTEGER, roleName TEXT, primaryColor TEXT, secondColor TEXT, urlImg TEXT, communityId INTEGER, communityName TEXT);');
  }
  ...
  Future<MyUser> getUser() async {
    ...
    final List<Map<String, dynamic>> maps =
        await dbConecction!.query(tbUsuarioLogIn);
    ...
    if (maps.isNotEmpty) {
      return List.generate(maps.length, (i) {
        return MyUser(
          ...
          communityName: maps[i]['communityName'],
        );
      })[0];
    } else {
      return emptyUser;
  ...
  void insertUser(MyUser usuarioLocal) async {
    ...
    int? nUsers = Sqflite.firstIntValue(
        await dbConnection!.rawQuery('SELECT COUNT(*) FROM $tbUsuarioLogIn'));
    if (nUsers! < 1) {
      String query =
          'INSERT INTO $tbUsuarioLogIn (idUser, nombre, lastname, telefono, correo, clave, energia, dinero, idCiudad, role, roleName, primaryColor, secondColor, urlImg, communityId, communityName) VALUES(\'${usuarioLocal.idUser}\', \'${usuarioLocal.nombre}\', \'${usuarioLocal.lastname}\', \'${usuarioLocal.telefono}\', \'${usuarioLocal.correo}\', \'${usuarioLocal.clave}\', \'${usuarioLocal.energia}\',\'${usuarioLocal.dinero}\', \'${usuarioLocal.idCiudad}\', \'${usuarioLocal.role}\', \'${usuarioLocal.roleName}\', \'${usuarioLocal.primaryColor}\', \'${usuarioLocal.secondColor}\', \'${usuarioLocal.urlImg}\', \'${usuarioLocal.communityId}\', \'${usuarioLocal.communityName}\')';
  ...
  Future<Usuarios> getUsers() async {
    var dbConecction = await db;
    final List<Map<String, dynamic>> maps =
        await dbConecction!.query(tbUsuarios);
  ...
  Future<void> addUsertbUsuarios(MyUser usuarioLocal) async {
    final Database? dbConnection = await db;
    int? nUsers = Sqflite.firstIntValue(
        await dbConnection!.rawQuery('SELECT COUNT(*) FROM $tbUsuarios'));
