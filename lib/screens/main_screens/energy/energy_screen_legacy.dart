...
class EnergyScreenLegacy extends StatefulWidget {
  final MyUser? myUser;
  const EnergyScreenLegacy({super.key, this.myUser});

  @override
  State<EnergyScreenLegacy> createState() => _EnergyScreenLegacyState();
}

class _EnergyScreenLegacyState extends State<EnergyScreenLegacy> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
          metodos.appbarEnergia(context, widget.myUser?.nombre ?? "Usuario"),
...
class LineChartSample extends StatelessWidget {
  ...
  LineChartData _buildChartData(BuildContext context) {
    ...
    final consumoSpots =
        data.map((d) => FlSpot(d.hour.toDouble(), d.consumption)).toList();

    // Serie 2: Producción Solar (línea sólida amarilla, sin área)
    final solarSpots =
        data.map((d) => FlSpot(d.hour.toDouble(), d.solarProd)).toList();

    // Serie 3: Autoconsumo (área verde bajo la línea de consumo)
    final autoconsumoSpots =
        data.map((d) => FlSpot(d.hour.toDouble(), d.selfConsumption)).toList();

    // Serie 4: Importada (área salmón)
    final importadaSpots =
        data.map((d) => FlSpot(d.hour.toDouble(), d.gridImport)).toList();

    // Serie 5: Exportada (área rosa/morada)
    final exportadaSpots =
        data.map((d) => FlSpot(d.hour.toDouble(), d.gridExport)).toList();
          ...
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (value, meta) {
              ...
              if (value == 0.0 ||
                ...
                return Text(Formatters.formatNumber(value, decimals: 1),
...
class _LineChart extends StatelessWidget {
  ...
  LineChartBarData _lineChartBarData2_1(BuildContext context) {
    ...
    final spots = dailyData.asMap().entries.map((entry) {
      ...
      return FlSpot(
          day.toDouble(), data.exported / 100); // Escalar para visualización
    }).toList();
  ...
  LineChartBarData _lineChartBarData2_2(BuildContext context) {
    ...
    final spots = dailyData.asMap().entries.map((entry) {
      ...
      final data = entry.value;
      return FlSpot(
          day.toDouble(), data.demand / 100); // Escalar para visualización
    }).toList();
    ...
    return LineChartBarData(
      ...
      belowBarData: BarAreaData(
        show: true,
        color:
            Color(int.parse("0xff${ColorsApp.color4}")).withValues(alpha: 0.2),
      ),
      spots: spots,
    );
  }
  ...
  LineChartBarData _lineChartBarData2_3(BuildContext context) {
    ...
    final spots = dailyData.asMap().entries.map((entry) {
      ...
      final data = entry.value;
      return FlSpot(
          day.toDouble(), data.imported / 100); // Escalar para visualización
    }).toList();
